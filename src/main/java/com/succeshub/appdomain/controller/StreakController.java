...
/**
 * REST API for the profile streak page: month activity calendar, shielded repair,
 * inventory card usage, and (when {@code succeshub.loot.enable-dev-grants} is true)
 * manual streak adjust/set/reset testing tools.
 */
@Tag(name = "Streak", description = "Streak calendar, shield/card usage, and gated testing tools")
@RestController
...
public class StreakController {
    ...
    public ResponseEntity<StreakCalendarDto> getCalendar(
        ...
        YearMonth target = month != null ? month : YearMonth.from(java.time.LocalDate.now());
    ...
    @Operation(summary = "Adjust streak",
            description = "Dev/testing tool: adds or removes days from the current streak; clamped at zero. "
                    + "Requires succeshub.loot.enable-dev-grants (typically the `dev` profile).")
     ...
     * Disabled unless {@code succeshub.loot.enable-dev-grants} is true.
     */
    @Operation(summary = "Reset streak",
            description = "Dev/testing tool: sets streak to zero and tier to NONE. "
                    + "Requires succeshub.loot.enable-dev-grants (typically the `dev` profile).")
